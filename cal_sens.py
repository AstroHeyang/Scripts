...
def generate_lc(satellite: str, instrument: str, filter_name: str, ao: int,
    ...
    if not alpha:
        alpha = np.random.uniform(1.9, 3.501)  # alpha in the range of [1.9, 3.5]
    if not temperature:
...
def cal_redshift(lc_file: str, rs_in: float, rs_range=None, satellite=None, instrument=None, filter_name=None,
    ...
    Args:
        ...
        index_obj: the index of this object in the object list; if not None, then this
            parameter is used to get the corresponding background count rate. This arg
            is used to get redshift using original parameters; if None, then random
            background count rates are used.
